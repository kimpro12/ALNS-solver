...
class Metrics:
    ...
    def append(
        ...
        if penalty_vec is None:
            ...
            penalty_vec = np.asarray(penalty_vec, dtype=np.float64)
        if penalty_weights is None:
            penalty_weights = np.zeros(F_PEN, dtype=np.float64)
        else:
            penalty_weights = np.asarray(penalty_weights, dtype=np.float64)
        if penalty_violations is None:
            penalty_violations = np.zeros(F_PEN, dtype=np.float64)
        else:
            penalty_violations = np.asarray(penalty_violations, dtype=np.float64)
