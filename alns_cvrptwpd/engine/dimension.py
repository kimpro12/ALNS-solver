...
def build_giant_tour_savings(dist: np.ndarray, nodes: np.ndarray) -> np.ndarray:
    ...
    size = n * (n - 1) // 2
    savings = np.empty(size, dtype=np.float64)
    pair_i = np.empty(size, dtype=np.int64)
    pair_j = np.empty(size, dtype=np.int64)
...
def repair_giant_tour_focus_njit(
    ...
    penalties = np.empty(savings_order.shape[0], dtype=np.float64)
