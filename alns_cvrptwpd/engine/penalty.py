...
def _violation_flags(vec, tol):
    flags = np.zeros(vec.shape[0], dtype=np.float64)
...
def init_penalty_state(params):
    ...
    state = {
        ...
        "min": min_w,
        "max": max_w,
        "history": np.zeros((F_PEN, history_size), dtype=np.float64),
        "hist_idx": np.zeros(F_PEN, dtype=np.int64),
        "hist_count": np.zeros(F_PEN, dtype=np.int64),
        "viol_ema": np.zeros(F_PEN, dtype=np.float64),
        "violation_alpha": violation_alpha,
        "update_rate": update_rate,
        "target": np.full(F_PEN, target, dtype=np.float64),
        "scale": np.full(F_PEN, scale_floor, dtype=np.float64),
        "scale_floor": scale_floor,
        "tol": np.full(F_PEN, tol, dtype=np.float64),
    }
    return state


def penalty_cost(state, pen_vec):
    pen_vec = np.asarray(pen_vec, dtype=np.float64)
