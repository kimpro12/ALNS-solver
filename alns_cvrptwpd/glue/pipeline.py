...
def run_pipeline(
    ...
    *,
    ...
    if export_trace:
        ...
        arr_iters = np.array([row[0] for row in metrics.rows], dtype=np.int64)
        arr_curr = np.array([row[1] for row in metrics.rows], dtype=np.float64)
        arr_best = np.array([row[2] for row in metrics.rows], dtype=np.float64)
        arr_temp = np.array([row[3] for row in metrics.rows], dtype=np.float64)
        np.savez(
            trace_path,
            iters=arr_iters,
            curr=arr_curr,
