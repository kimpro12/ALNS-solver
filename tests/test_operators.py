...
def _make_node_indices(n):
    return np.zeros((n, F_NODE_I), dtype=np.int64)
...
def test_best_insertion_selects_lowest_delta_route():
    routes = np.array([[1, 0, 0], [2, 0, 0]], dtype=np.int64)
    lens = np.array([1, 1], dtype=np.int64)
    loads = np.array([1.0, 1.0], dtype=np.float64)
    dist = np.array(
        [
            [0.0, 1.0, 3.0, 1.0],
            [1.0, 0.0, 10.0, 10.0],
    ...
    unrouted = np.array([3], dtype=np.int64)
...
def test_best_insertion_respects_time_windows():
    routes = np.array([[1, 0, 0]], dtype=np.int64)
    lens = np.array([1], dtype=np.int64)
    loads = np.array([1.0], dtype=np.float64)
    dist = np.array(
        [
            [0.0, 2.0, 2.0],
            [2.0, 0.0, 10.0],
    ...
    unrouted = np.array([2], dtype=np.int64)
    inserted, used_mask, changed = best_insertion(
        unrouted,
        routes,
        lens,
